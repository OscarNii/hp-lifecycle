...
export default function Sidebar() {
  ...
  const handleProductClick = (product: HPProduct) => {
    setSearchQuery('');
    setShowResults(false);
    setIsMobileOpen(false);
    router.push(`/version-explorer?product=${product.id}`);
  };
  ...
  const getStatusColor = (status: string) => {
    switch (status) {
      ...
      case 'End of Life': return 'bg-red-500/20 text-red-400 border-red-500/30';
    ...
    <>
      ...
      `}>
        ...
        <div className="p-3 border-t border-white/10">
          {/* Live Quick Search */}
          {!isCollapsed && (
            <div className="mb-3 px-2 relative" ref={searchRef}>
              ...
              {showResults && searchResults.length > 0 && (
                <div className="absolute left-2 right-2 mt-2 bg-[#0d2545]/95 backdrop-blur-xl rounded-xl shadow-2xl border border-white/20 z-50 max-h-64 overflow-y-auto">
                    ...
                    >
                      <div className="flex items-center justify-between">
                        <div>
                          ...
                          <p className="text-white/50 text-xs">{product.series} • {product.category}</p>
                        </div>
                        <span className={`px-2 py-0.5 rounded-full text-xs font-medium border ${getStatusColor(product.lifecycle.status)}`}>
              ...
              {showResults && searchQuery.length >= 2 && searchResults.length === 0 && (
                <div className="absolute left-2 right-2 mt-2 bg-[#0d2545]/95 backdrop-blur-xl rounded-xl shadow-2xl border border-white/20 z-50 p-4 text-center">
